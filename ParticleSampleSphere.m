...
%              default setting. A warning will be issued is N exceeds 1E3. 
...
while i<Nitr && dE>Etol
    ...
    % Evaluate the total potential energy of the system 
    Ue_i=sum(Ue_ij,2);
    Ue(i+1)=sum(Ue_i);
    ...
    switch id
        case 1 % number of particles
            ...
            if sum(Val(:)<10)>0 || ~isreal(Val) || ~isnumeric(Val) || numel(Val)~=1 
                error('Incorrect entry for the ''%s'' option. N must be a positive interger greater than 9.',FNo{1})
            end
